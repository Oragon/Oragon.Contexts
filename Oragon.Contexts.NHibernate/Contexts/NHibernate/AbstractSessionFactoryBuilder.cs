...
namespace Oragon.Contexts.NHibernate
{
    ...
    public abstract class AbstractSessionFactoryBuilder : ISessionFactoryBuilder
    {
        ...
        public NH.ISessionFactory BuildSessionFactory()
        {
            ...
            {
                semaphore.Acquire();
                if (sessionFactory == null)//Reteste.. outra thread pode ter feito o preenchimento do campo antes da liberação do semáfoto.
                {
                    try
                    {
                        sessionFactory = BuildSessionFactoryInternal();
                    }
                    catch (Exception)
                    {
                        throw;
